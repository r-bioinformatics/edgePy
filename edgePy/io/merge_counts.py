...
def merge(dir_path):
	...
	"""
	files_to_merge = [f for f in os.listdir(dir_path) if f.endswith('.tsv')]
	_df = pd.read_csv(dir_path+files_to_merge[0], sep='\t',names=['id','count'], index_col=0)
	final_df = pd.DataFrame(data=None, index=_df.index)
	for f in files_to_merge: 
		df_to_add = pd.read_csv(dir_path+f, sep='\t', index_col=0, comment='_', names=[f.split('.')[0].split('/')[-1]]) 
